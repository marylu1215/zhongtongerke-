...
- (void)setupView {
    ...
    [bottomView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        CGFloat height = (self.title.length > 0 ? self.titleTextSize.height + 28.0f : 0) + (self.otherButtonTitles.count > 0 ? (self.canScrolling ? MIN(self.visibleButtonCount, self.otherButtonTitles.count) : self.otherButtonTitles.count) * self.buttonHeight : 0) + (self.cancelButtonTitle.length > 0 ? 0.0f + self.buttonHeight : 0);
...
//    tableView.scrollEnabled = NO;
    ...
    divisionView.alpha           = 1.0f;
    divisionView.backgroundColor = LC_ACTION_SHEET_COLOR(203, 203, 203);
    ...
    [divisionView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        CGFloat height = self.cancelButtonTitle.length > 0 ? 1.0f : 0;
        make.height.equalTo(@(height));
    }];
...
- (void)cancelButtonClicked {
    if ([self.delegate respondsToSelector:@selector(actionSheet:clickedButtonAtIndex:)]) {
        [self.delegate lcactionSheet:self clickedButtonAtIndex:0];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.delegate respondsToSelector:@selector(actionSheet:clickedButtonAtIndex:)]) {
        [self.delegate lcactionSheet:self clickedButtonAtIndex:indexPath.row + temp];
    }
