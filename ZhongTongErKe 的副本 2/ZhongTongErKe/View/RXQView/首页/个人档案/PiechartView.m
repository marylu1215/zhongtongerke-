...
-(id)initWithFrame:(CGRect)frame withStrokeWidth:(CGFloat )width andColor:(UIColor *)color andPercent:(CGFloat)percent andAnimation:(BOOL) animation
{
    ...
    if (self) {
        ...
        _circlePath = [UIBezierPath bezierPathWithArcCenter:centerPoint radius:radius startAngle:-M_PI_2 endAngle:M_PI_2*3 clockwise:YES];
...
-(void)percentAnimation
{
    ...
    strokeEndAnimation.autoreverses = NO; //无自动动态倒退
