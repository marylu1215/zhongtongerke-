...
- (void)sendRegistRequest
{
    ...
    [HttpTool postWithURL:registUrl params:params success:^(id json) {
        
        if (jsonMsg(json)) {
            
            if (self.registerSucessBlock) {
                self.registerSucessBlock(self.telTextField.text,self.pwdTextField.text,NO);
            }
        ...
        NSLog(@"错误error:%@",error.description);
        kHideHUD;
        netWorkError;
    }];
